...
## Адрес сервера с развернутым приложением: http://130.193.56.127/

## Настройка сервера:
- В nginx/nginx.conf в разделе http { server { server_name } } указывается ip адрес по которому будет доступно веб-приложение.
  - пример 
  http { 
  server { 
  server_name 130.193.56.127; 
  } 
  }
